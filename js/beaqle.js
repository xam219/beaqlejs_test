...
//        this.TestState.Runtime.forEach(function(element, index, array){array[index] = 0});
    ...
    ListeningTest.prototype.SubmitTestResults = function () {
        ...
        // Google Form 的 URL
            var formUrl = this.TestConfig.BeaqleServiceURL;
            ...
            fetch(formUrl, {
                ...
                $('#SubmitBox').html("Error submitting results! <br/><br/>");
                testHandle.TestState.TestIsRunning = 0;
            });
...
MushraTest.prototype.createTestDOM = function (TestIdx) {
    ...
    for (var i = 0; i < this.TestState.FileMappings[TestIdx].length; i++) { 
        ...
        var fileID = this.TestState.FileMappings[TestIdx][i];
        var relID  = (fileID === "Reference") ? "HiddenRef" : fileID;
        ...
        cell[2].innerHTML = "<button class='stopButton'>Stop</button>";  
        cell[3] = row[i].insertCell(-1);
        var fileIDstr = (this.TestConfig.ShowFileIDs) ? fileID : "";

        // 添加评分控件
        cell[3].innerHTML = "<div class='rateSlider' id='slider"+fileID+"' rel='"+relID+"'>"+fileIDstr+"</div>";
        cell[4] = row[i].insertCell(-1);
        cell[4].innerHTML = "<label for='qualityInput"+fileID+"'>Speech Quality:</label><input type='number' id='qualityInput"+fileID+"' min='1' max='5'>";
        cell[5] = row[i].insertCell(-1);
        cell[5].innerHTML = "<label for='intelligibilityInput"+fileID+"'>Intelligibility:</label><input type='number' id='intelligibilityInput"+fileID+"' min='1' max='5'>";
